...
const mockPatients = {
    "P1001": {
        ...
        prescriptions: ["PARACETAMOL", "METFORMIN", "AMLODIPINE"],
...
const mockMedicines = {
    ...
    "AMLODIPINE": {
        name: "Amlodipine",
        contents: "Amlodipine Besylate 5mg",
        purpose: "Blood pressure control",
        dosage: "1 Tablet",
        schedule: "Once daily in the morning",
        warnings: "May cause dizziness. Avoid sudden posture changes.",
        whoCanConsume: "Adults with Hypertension",
...
const dietDatabase = {
    "PARACETAMOL": {
        eat: ["Soup", "Crackers", "Bananas", "Rice", "Applesauce", "Toast"],
        avoid: ["Alcohol", "Spicy food", "Heavy greasy food"]
    },
    "METFORMIN": {
        eat: ["Spinach", "Chicken breast", "Oatmeal", "Brown rice", "Broccoli", "Fish"],
        avoid: ["Sugary drinks", "White bread", "Alcohol", "Grapefruit", "Sweets"]
    },
    "ALBUTEROL": {
        eat: ["Almonds", "Spinach", "Salmon", "Oranges", "Apples"],
        avoid: ["Coffee", "Energy drinks", "Wine", "Dried fruits"]
    },
    "AMLODIPINE": {
        eat: ["Bananas", "Spinach", "Sweet potatoes", "Beans", "Fish"],
        avoid: ["Salty food", "Soy sauce", "Licorice", "Processed meats"]
    },
    "DEFAULT": {
        eat: ["Chicken", "Rice", "Vegetables", "Water", "Fruits"],
        avoid: ["Junk food", "Alcohol", "Excessive sugar"]
    }
};
...
db.ref('patients').on('value', snap => { 
    ...
    // Auto-update Patient UI if logged in
    if (state.currentRole === 'patient' && state.currentUserId) {
        ...
        if (greetingEl) greetingEl.textContent = `Welcome, ${state.currentUser.name}`;
        
        // Update sub-views if they are active
        if(document.getElementById('v-medicines').classList.contains('active')) renderMedicinesList();
        if(document.getElementById('v-appointments').classList.contains('active')) renderAppointments();
    }
    
    // Auto-update Doctor Queue names and dropdowns if data changed
    if (state.currentRole === 'doctor') {
        renderDoctorQueue();
        if(document.getElementById('v-doc-manage').classList.contains('active')) populateDoctorManageDropdowns();
    }
});

db.ref('medicines').on('value', snap => { 
    if(snap.val()) liveMedicines = snap.val(); 
    // Auto-update Doctor dropdowns if active
    if (state.currentRole === 'doctor') {
        if(document.getElementById('v-doc-manage').classList.contains('active')) populateDoctorManageDropdowns();
        if(document.getElementById('v-doc-qr').classList.contains('active')) populateQRDropdown();
    }
    // Auto-update Patient medicine list if active
    if (state.currentRole === 'patient' && document.getElementById('v-medicines').classList.contains('active')) {
        renderMedicinesList();
    }
});

db.ref('doctors').on('value', snap => { 
    if(snap.val()) liveDoctors = snap.val(); 
    if (state.currentRole === 'doctor' && state.currentUserId) {
        state.currentUser = liveDoctors[state.currentUserId];
        const docGreetingEl = document.getElementById('doc-dashboard-greeting');
        if (docGreetingEl) docGreetingEl.textContent = `Welcome, ${state.currentUser.name}`;
    }
});
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Diet & Nutrition Nav
    const btnNavDiet = document.getElementById('btn-nav-diet');
    if (btnNavDiet) {
        btnNavDiet.addEventListener('click', () => {
            triggerVibrate(100);
            speakText("Diet and Nutrition Plan.");
            renderDietRecommendations();
            switchView('v-diet');
        });
    }
    ...
    // Handle patient selection change to pre-fill diet fields
    document.getElementById('manage-pat-select').addEventListener('change', (e) => {
        const patId = e.target.value;
        const eatInput = document.getElementById('manage-diet-eat');
        const avoidInput = document.getElementById('manage-diet-avoid');
        
        if (livePatients[patId] && livePatients[patId].customDiet) {
            eatInput.value = livePatients[patId].customDiet.eat || "";
            avoidInput.value = livePatients[patId].customDiet.avoid || "";
        } else {
            eatInput.value = "";
            avoidInput.value = "";
        }
    ...
    document.getElementById('btn-save-diet').addEventListener('click', () => {
        const patId = document.getElementById('manage-pat-select').value;
        const eatText = document.getElementById('manage-diet-eat').value.trim();
        const avoidText = document.getElementById('manage-diet-avoid').value.trim();
        ...
        if(!eatText && !avoidText) {
            // If both empty, remove the custom diet node
            db.ref('patients/' + patId + '/customDiet').remove();
        } else {
            db.ref('patients/' + patId + '/customDiet').set({
                eat: eatText,
                avoid: avoidText
            });
        }
        
        const feedback = document.getElementById('diet-feedback');
        feedback.classList.remove('hidden');
        triggerVibrate(100);
        setTimeout(() => feedback.classList.add('hidden'), 2000);
    });
...
// ==========================================
// RENDER: DIET & NUTRITION
// ==========================================
function renderDietRecommendations() {
    const listEl = document.getElementById('diet-list');
    if (!listEl) return;
    
    listEl.innerHTML = '';
    
    // Check for Doctor's Custom Diet Orders first
    if (state.currentUser.customDiet && (state.currentUser.customDiet.eat || state.currentUser.customDiet.avoid)) {
        const customEat = state.currentUser.customDiet.eat ? `<li class="flex items-start"><i class="fa-solid fa-check text-green-500 mt-1 mr-2"></i><span>${state.currentUser.customDiet.eat}</span></li>` : '';
        const customAvoid = state.currentUser.customDiet.avoid ? `<li class="flex items-start"><i class="fa-solid fa-xmark text-red-500 mt-1 mr-2"></i><span>${state.currentUser.customDiet.avoid}</span></li>` : '';
        
        listEl.innerHTML += `
            <div class="bg-amber-50 border-4 border-amber-300 rounded-xl overflow-hidden shadow-lg mb-6">
                <div class="bg-amber-100 p-4 border-b border-amber-300 flex items-center">
                    <i class="fa-solid fa-user-doctor text-amber-700 mr-3 text-2xl"></i>
                    <h3 class="font-black text-amber-900 text-xl flex-grow">Doctor's Custom Orders</h3>
                </div>
                
                <div class="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="bg-white rounded-lg p-4 border-2 border-green-200">
                        <h4 class="font-bold text-green-800 uppercase text-sm tracking-wider mb-2 flex items-center">
                            <i class="fa-solid fa-utensils mr-2"></i> Prescribed to Eat
                        </h4>
                        <ul class="text-md text-slate-800 space-y-2 font-bold">
                            ${customEat || '<span class="text-slate-400 italic font-normal">No specific foods prescribed.</span>'}
                        </ul>
                    </div>
                    
                    <div class="bg-white rounded-lg p-4 border-2 border-red-200">
                        <h4 class="font-bold text-red-800 uppercase text-sm tracking-wider mb-2 flex items-center">
                            <i class="fa-solid fa-ban mr-2"></i> Prescribed to Avoid
                        </h4>
                        <ul class="text-md text-slate-800 space-y-2 font-bold">
                            ${customAvoid || '<span class="text-slate-400 italic font-normal">No specific foods to avoid.</span>'}
                        </ul>
                    </div>
                </div>
            </div>
            <div class="text-center text-slate-400 font-bold mb-4 uppercase tracking-wider text-xs border-b border-slate-200 pb-2">Automated Medicine-Based Suggestions</div>
        `;
    }
    
    const prescriptions = state.currentUser.prescriptions || [];
    
    if (prescriptions.length === 0) {
        listEl.innerHTML += `
            <div class="bg-slate-50 border-2 border-slate-200 rounded-xl p-8 text-center">
                <i class="fa-solid fa-apple-whole text-4xl text-slate-300 mb-4 block"></i>
                <p class="text-xl font-bold text-slate-500">No active prescriptions.</p>
                <p class="text-slate-400 mt-2">Maintain a standard healthy diet and stay hydrated.</p>
            </div>
        `;
        return;
    }

    prescriptions.forEach(medId => {
        const medData = liveMedicines[medId];
        if (!medData) return;
        
        // Lookup the diet profile, or use DEFAULT if not found
        const dietProfile = dietDatabase[medId] || dietDatabase["DEFAULT"];
        
        let eatHtml = '';
        dietProfile.eat.forEach(item => {
            eatHtml += `<li class="flex items-start"><i class="fa-solid fa-check text-green-500 mt-1 mr-2"></i><span>${item}</span></li>`;
        });
        
        let avoidHtml = '';
        dietProfile.avoid.forEach(item => {
            avoidHtml += `<li class="flex items-start"><i class="fa-solid fa-xmark text-red-500 mt-1 mr-2"></i><span>${item}</span></li>`;
        });

        const cardHtml = `
            <div class="bg-white border-2 border-green-200 rounded-xl overflow-hidden shadow-sm mb-4">
                <div class="bg-green-100 p-3 border-b border-green-200 flex items-center">
                    <i class="fa-solid fa-pills text-green-700 mr-3 text-xl"></i>
                    <h3 class="font-bold text-green-900 text-lg flex-grow">Diet for: ${medData.name}</h3>
                </div>
                
                <div class="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <!-- Eat Section -->
                    <div class="bg-emerald-50 rounded-lg p-3 border border-emerald-100">
                        <h4 class="font-bold text-emerald-800 uppercase text-xs tracking-wider mb-2 flex items-center">
                            <i class="fa-solid fa-thumbs-up mr-2"></i> Recommended
                        </h4>
                        <ul class="text-sm text-emerald-900 space-y-2 font-medium">
                            ${eatHtml}
                        </ul>
                    </div>
                    ...
                    <div class="bg-red-50 rounded-lg p-3 border border-red-100">
                        <h4 class="font-bold text-red-800 uppercase text-xs tracking-wider mb-2 flex items-center">
                            <i class="fa-solid fa-triangle-exclamation mr-2"></i> Avoid
                        </h4>
                        <ul class="text-sm text-red-900 space-y-2 font-medium">
                            ${avoidHtml}
                        </ul>
                    </div>
                </div>
